...
class RestaurantCard extends React.Component {
    ...
    render(){
        const { name, address, city, state, postal_code, business_id, reviews } = this.props;
            ...
            <StyledCard>
                ...
                <div className="restaurant-info">
                    ...
                    <>
                        ...
                        <div>{ reviews ? reviews.map( (review, ind) => <div key={ind}>{review.penName}: {review.review}</div>) : null }</div>
                    </>
                    }
                </div>
                {dislike ? null : <span onClick={() => this.setSelection('like')} role="img" aria-label="yes">❤️</span>}
                <ReviewForm handleNewReview={this.props.handleNewReview} restId={business_id}/>
            </StyledCard>
        )
    }
}
